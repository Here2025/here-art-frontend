...
const fallbackProfiles = [
  {
    id: 'p1',
    displayName: 'HERE City Curator',
    handle: 'here-curator',
    profileType: 'curator',
    bio: 'A curator profile showing how HERE connects street art, public art, events, profiles, and journeys.',
    city: 'Raleigh',
    state: 'NC',
    website: '',
    imageUrl: people[0],
  },
  {
    ...
    bio: 'A visual storyteller creating public art, murals, and city-based work.',
    city: 'Raleigh',
    state: 'NC',
    website: '',
    imageUrl: people[1],
  ...
  {
    id: 'p3',
    displayName: 'Oak City Sound',
    handle: 'oakcitysound',
    profileType: 'performing_artist',
    bio: 'A creative host profile for local performances, pop-ups, and cultural events.',
    city: 'Raleigh',
    state: 'NC',
    website: '',
    imageUrl: people[2],
...
const fallbackArtworks = [
  {
    id: 'a1',
    title: 'City Wall Study',
    artist: 'Local muralist',
    description: 'A street-art inspired placeholder showing how HERE highlights creative work connected to place.',
  ...
  {
    id: 'a2',
    title: 'Public Color Study',
    artist: 'Public artist',
    description: 'A public-art inspired placeholder for creative moments people can discover while moving through the city.',
  ...
  },
  {
    ...
    description: 'A gallery-inspired placeholder while HERE expands its verified creative content and image-rights workflow.',
    address: 'Raleigh, NC',
...
const fallbackEvents = [
  {
    ...
    eventType: 'Art Walk',
    description: 'A guided creative walk through public art, murals, and hidden creative places.',
  ...
  {
    id: 'e2',
    title: 'Gallery Night & Local Sounds',
    eventType: 'Creative Event',
    description: 'A creative evening connecting visual art, sound, and people who want to discover local talent.',
...
const fallbackJourneys = [
  {
    id: 'j1',
    title: 'Downtown Creative Walk',
    description: 'A curated trail through murals, public art, galleries, and hidden creative places.',
    city: 'Raleigh',
    state: 'NC',
    curatorProfileId: 'p1',
    imageUrl: art[4],
...
function mapUrl(x) {
  ...
  if (x?.address || x?.venueName) {
    return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(x.address || x.venueName)}`;
  }
  return '';
}
...
export default function App() {
  ...
  const selectedArt = artworks.find((x) => x.id === artId) || artworks[0] || fallbackArtworks[0];
  ...
  useEffect(() => {
    ...
    artworks.filter((a) => a.lat && a.lng).forEach((a) => {
      const selected = a.id === selectedArt.id;
      const icon = window.L.divIcon({
        className: `here-map-pin ${selected ? 'selected' : ''}`,
        html: `<span>${initials(a.category).slice(0, 1)}</span>`,
        iconSize: [42, 42],
        iconAnchor: [21, 21],
      });
    ...
    if (selectedArt.lat && selectedArt.lng) {
      mapRef.current.setView([selectedArt.lat, selectedArt.lng], 14, { animate: true });
    }
  }, [page, artworks, selectedArt]);
      ...
      <main className="app-page-wrap">
        {page === 'discover' && (
          <DiscoverPage
            ...
            save={(id) => flip(setSaved, id)}
            like={(id) => flip(setLiked, id)}
            openArt={openArt}
            openEvent={openEvent}
            openMap={() => setPage('map')}
            openEvents={() => setPage('events')}
          />
        )}
        {page === 'map' && (
          <MapPage
            ...
            saved={saved.has(selectedArt.id)}
            visited={visited.has(selectedArt.id)}
            setSelected={setArtId}
            save={() => flip(setSaved, selectedArt.id)}
            check={() => flip(setVisited, selectedArt.id)}
          />
        )}
        {page === 'events' && (
          <EventsPage
            ...
            save={() => flip(setSaved, selectedEvent.id)}
            openEvent={openEvent}
        ...
        {page === 'profile' && (
          <ProfilePage
            ...
            journeys={profileJourneys}
            followed={following.has(selectedProfile.id)}
            follow={() => flip(setFollowing, selectedProfile.id)}
            openProfile={openProfile}
            openEvent={openEvent}
          />
        )}
        {page === 'saved' && (
          <SavedPage
            savedArtworks={artworks.filter((a) => saved.has(a.id))}
            savedEvents={events.filter((e) => saved.has(e.id))}
            likedArtworks={artworks.filter((a) => liked.has(a.id))}
            checkIns={artworks.filter((a) => visited.has(a.id))}
            followed={profiles.filter((p) => following.has(p.id))}
            openArt={openArt}
            openEvent={openEvent}
            openProfile={openProfile}
          />
...
function AppHeader({ page, setPage }) {
  return (
    <header className="here-app-header">
      <button className="here-brand" onClick={() => setPage('discover')} type="button">
        <span>H</span>
        <strong>HERE</strong>
      </button>
      <p>Find art where you are.</p>
      <nav className="desktop-nav">
        {['discover', 'map', 'events', 'create', 'saved', 'profile'].map((x) => (
          <button className={page === x ? 'active' : ''} key={x} onClick={() => setPage(x)} type="button">
            {x === 'discover' ? 'Discover' : x === 'saved' ? 'My Space' : x[0].toUpperCase() + x.slice(1)}
          </button>
        ))}
      </nav>
    </header>
      ...
      <div className="discover-sidebar">
        ...
        <div className="category-grid-real category-grid-expanded">
          {visibleCategories.map((x, i) => (
            <button
              className={cat === x.label ? 'active' : ''}
              key={x.label}
              onClick={() => x.label === 'Creative Event' ? openEvents() : setCat(x.label)}
              type="button"
            >
              <ImageTile item={{ imageUrl: art[(i + 1) % art.length], title: x.label }} />
              <span>{x.label}</span>
            </button>
          ))}
        </div>

        <SectionHeader title={query ? 'Search results' : 'Creative events'} action="See events" onAction={openEvents} />
        <div className="content-grid three">
          {(evs.length ? evs : events).slice(0, 3).map((e) => <EventCard key={e.id} event={e} onClick={() => openEvent(e)} />)}
          {!evs.length && query && <p className="empty-text">No matching events yet. Try a place, category, or artist search.</p>}
        </div>
...
function MapPage({ box, artworks, selected, saved, visited, setSelected, save, check }) {
  const [q, setQ] = useState('');
  const [category, setCategory] = useState('All');
  const url = mapUrl(selected);
  const list = artworks.filter((a) => match(a, q) && matchCat(a, category));
  ...
  return (
    <section className="page map-page">
      <div className="page-title-row">
        <div>
          <h1>Creative Map</h1>
          <p>Explore street art, public art, galleries, stages, fashion, and hidden creative places connected to location.</p>
        </div>
        <SearchBar text="Search this area" value={q} onChange={setQ} />
      </div>
      <div className="map-filter-row">
        {categoryOptions.map((option) => (
          <button className={category === option.label ? 'active' : ''} key={option.label} onClick={() => setCategory(option.label)} type="button">
            {option.label}
          </button>
        ))}
      </div>
      <div className="map-grid">
        <div className="map-canvas" ref={box} role="application" aria-label="HERE map" />
        <aside className="selected-place-card">
          <ImageTile item={selected} />
          <p className="label">Selected creative place</p>
          <h2>{selected.title}</h2>
          <small>{selected.category || 'Creative place'} · {selected.artist || 'Artist / place details'}</small>
          <p>{selected.description}</p>
          <span>{selected.address || selected.neighborhood}</span>
          <div className="button-row">
            <button className={saved ? 'active' : ''} onClick={save} type="button">{saved ? 'Saved' : 'Save place'}</button>
            <button className={visited ? 'active' : ''} onClick={check} type="button">{visited ? 'Checked in' : 'Check in'}</button>
            {url && <a href={url} target="_blank" rel="noreferrer">Open location</a>}
          </div>
          <SectionHeader title="Nearby creative places" />
          <div className="mini-list">
            {list.slice(0, 5).map((a) => (
              <button className={a.id === selected.id ? 'active' : ''} onClick={() => setSelected(a.id)} key={a.id} type="button">
                <ImageTile item={a} />
                <span>
                  <strong>{a.title}</strong>
                  <small>{a.category || 'Creative place'} · {a.neighborhood || a.address}</small>
                </span>
              </button>
            ))}
            {!list.length && <p className="empty-text">No matching places yet. Clear search or choose All.</p>}
          </div>
        </aside>
      </div>
    </section>
  ...
  return (
    <section className="page event-page">
      ...
      <article className="event-detail-panel">
        ...
        <h1>{event.title}</h1>
        <InfoLine label={dateText(event.startsAt)} />
        <InfoLine label={timeText(event.startsAt, event.endsAt)} />
        ...
        <button className="host-strip-real" onClick={() => openProfile(host)} type="button">
          <Avatar profile={host} />
          <span><strong>{host?.displayName || 'Host profile'}</strong><small>{typeLabel(host?.profileType || 'Creative host')}</small></span>
          <em>View profile</em>
        </button>
        <div className="button-row">
          <button className={saved ? 'active' : ''} onClick={save} type="button">{saved ? 'Saved' : 'Save'}</button>
          {url && <a href={url} target="_blank" rel="noreferrer">Open location</a>}
          {event.ticketUrl && <a href={event.ticketUrl} target="_blank" rel="noreferrer">Website</a>}
        </div>
      </article>
      <section className="full-row">
        <SectionHeader title="More creative events" />
        <div className="content-grid three">
          {events.filter((x) => x.id !== event.id).slice(0, 3).map((x) => <EventCard key={x.id} event={x} onClick={() => openEvent(x)} />)}
        </div>
      </section>
    </section>
  );
  ...
  return (
    <section className="page profile-page">
      ...
      <div className="profile-gallery-panel">
        ...
        {tab === 'artworks' && <div className="profile-art-grid-real">{artworks.slice(0, 6).map((a) => <ImageTile key={a.id} item={a} />)}</div>}
        {tab === 'events' && <div className="content-grid three">{(events.length ? events : fallbackEvents).slice(0, 6).map((e) => <EventCard key={e.id} event={e} onClick={() => openEvent(e)} />)}</div>}
        {tab === 'journeys' && <div className="content-grid three">{(journeys.length ? journeys : fallbackJourneys).slice(0, 6).map((j) => <JourneyCard key={j.id} journey={j} />)}</div>}
      </div>
      <section className="full-row">
        <SectionHeader title="More creators" />
        <div className="creator-row">
          {profiles.filter((x) => x.id !== profile.id).slice(0, 5).map((x) => (
            <button key={x.id} onClick={() => openProfile(x)} type="button"><Avatar profile={x} /><span>{x.displayName}</span></button>
          ))}
        </div>
      </section>
    </section>
...
function SavedPage({ savedArtworks, savedEvents, likedArtworks, checkIns, followed, openArt, openEvent, openProfile }) {
  ...
  const sections = [
    { id: 'places', title: 'Saved Places', count: savedArtworks.length },
    { id: 'events', title: 'Saved Events', count: savedEvents.length },
    { id: 'liked', title: 'Liked Art', count: likedArtworks.length },
    { id: 'following', title: 'Following', count: followed.length },
    { id: 'checkins', title: 'My Check-ins', count: checkIns.length },
  ];

  return (
    <section className="page saved-page interactive-saved">
      <div className="page-title-row"><h1>My Space</h1><p>Your saved places, liked art, followed creators, and check-ins in one place.</p></div>
      <div className="saved-tabs">
        {sections.map((s) => <button className={active === s.id ? 'active' : ''} key={s.id} onClick={() => setActive(s.id)} type="button"><strong>{s.count}</strong><span>{s.title}</span></button>)}
      </div>
      <div className="saved-active-panel">
        {active === 'places' && <SavedSection title="Saved Places" empty="Save places from Discover or Map and they will appear here." items={savedArtworks} render={(a) => <SavedTile key={a.id} item={a} onClick={() => openArt(a)} />} />}
        {active === 'events' && <SavedSection title="Saved Events" empty="Saved creative events will appear here." items={savedEvents} render={(e) => <SavedTile key={e.id} item={e} onClick={() => openEvent(e)} />} />}
        {active === 'liked' && <SavedSection title="Liked Art" empty="Liked artwork will appear here." items={likedArtworks} render={(a) => <SavedTile key={a.id} item={a} onClick={() => openArt(a)} />} />}
        {active === 'following' && <section className="saved-section"><SectionHeader title="Following" />{followed.length ? <div className="creator-row">{followed.map((p) => <button key={p.id} onClick={() => openProfile(p)} type="button"><Avatar profile={p} /><span>{p.displayName}</span></button>)}</div> : <p className="empty-text">Follow an artist, host, gallery, or collective and they will appear here.</p>}</section>}
        {active === 'checkins' && <SavedSection title="My Check-ins" empty="Check in at places you visit to build your memory trail." items={checkIns} render={(a) => <SavedTile key={a.id} item={a} onClick={() => openArt(a)} />} />}
      </div>
    </section>
  );
}
  ...
  return (
    <nav className="bottom-app-nav">
      {[
        ...
        ['saved', 'Saved'],
        ['profile', 'Profile'],
      ].map(([p, l]) => <button className={page === p ? 'active' : ''} onClick={() => setPage(p)} key={p} type="button"><span>{l[0]}</span>{l}</button>)}
    </nav>
  );
}
...
function FeatureArtwork({ artwork, onClick }) {
  return (
    <button className="feature-real" onClick={onClick} type="button">
      <ImageTile item={artwork} />
      <div><span>Street / public art spotlight</span><strong>{artwork.title}</strong><small>{artwork.category || artwork.address || artwork.neighborhood}</small></div>
      <em>Open map</em>
    </button>
  );
}
...
function ArtworkCard({ artwork, saved, liked, save, like, onClick }) {
  return (
    <article className="content-card">
      <button onClick={onClick} type="button"><ImageTile item={artwork} /><strong>{artwork.title}</strong><small>{artwork.category || artwork.neighborhood || artwork.address}</small></button>
      <div><button className={saved ? 'active' : ''} onClick={save} type="button">{saved ? 'Saved' : 'Save'}</button><button className={liked ? 'active' : ''} onClick={like} type="button">{liked ? 'Liked' : 'Like'}</button></div>
    </article>
  );
}
