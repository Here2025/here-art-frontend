...
export default function AddCreatePage({ api, onNotice }) {
  ...
  function validate() {
    if (mode === 'artwork' && !active.title.trim()) return 'Add a title for the artwork or place.';
    if (mode === 'event' && !active.title.trim()) return 'Add an event title.';
    if (mode === 'profile' && !active.displayName.trim()) return 'Add a profile name.';
    return '';
  }
  ...
  async function postSubmission(path, payload) {
    ...
    if (!response.ok) throw new Error(`Unable to submit to ${path}`);
    ...
    <section className="page add-create-page">
      <div className="add-create-intro">
        ...
        <div className="create-mode-tabs">
          <button className={mode === 'artwork' ? 'active' : ''} onClick={() => setMode('artwork')} type="button">Artwork / Place</button>
          <button className={mode === 'event' ? 'active' : ''} onClick={() => setMode('event')} type="button">Event</button>
...
function ArtworkFields({ value, update }) {
  return <>
    <Field label="Title"><input value={value.title} onChange={(e) => update('title', e.target.value)} placeholder="Example: Color Story Wall" /></Field>
    <Field label="Artist / host"><input value={value.artist} onChange={(e) => update('artist', e.target.value)} placeholder="Artist, gallery, or unknown" /></Field>
    <Field label="Category"><select value={value.category} onChange={(e) => update('category', e.target.value)}><option>Mural</option><option>Street Art</option><option>Gallery</option><option>Installation</option><option>Exhibition</option><option>Hidden Gem</option></select></Field>
    <Field label="Image URL"><input value={value.imageUrl} onChange={(e) => update('imageUrl', e.target.value)} placeholder="Image upload comes next" /></Field>
    <Field label="Address" wide><input value={value.address} onChange={(e) => update('address', e.target.value)} placeholder="Street, landmark, or neighborhood" /></Field>
    ...
    <Field label="Latitude"><input value={value.lat} onChange={(e) => update('lat', e.target.value)} placeholder="35.7796" /></Field>
    <Field label="Longitude"><input value={value.lng} onChange={(e) => update('lng', e.target.value)} placeholder="-78.6382" /></Field>
    <Field label="Description" wide><textarea value={value.description} onChange={(e) => update('description', e.target.value)} placeholder="What should people notice about this place?" /></Field>
  </>;
}
